...
Number *Complex::add(Number * num) {
	...
	if (num->types() == INTEGER) {
		Integer* aNum = dynamic_cast<Integer*>(num);
		this->value.first += aNum->val();
		resultNum = this;
		delete(num);
	} else if (num->types() == FLOAT) {
		Float* aNum = dynamic_cast<Float*>(num);
		this->value.first += aNum->val();
		resultNum = this;
		delete(num);
	} else if (num->types() == COMPLEX) {
		Complex* aNum = dynamic_cast<Complex*>(num);
		double realSeg = this->value.first + aNum->val().first;
		double imagSeg = this->value.second + aNum->val().second;
		resultNum = new Complex(realSeg, imagSeg);
		delete(num);
		delete(this);
	} else {
		resultNum = nullptr;
...
Number *Complex::mul(Number * num) {
	...
	if (num->types() == INTEGER) {
		...
		Complex* aNum = dynamic_cast<Complex*>(num);
		double realSeg = this->value.first * aNum->val().first - this->value.second * aNum->val().second;
		double imagSeg = this->value.first * aNum->val().second + this->value.second * aNum->val().first;
		resultNum = new Complex(realSeg, imagSeg);
		delete(num);
		delete(this);
	} else {
...
double Complex::imag() const {
     ...
     * 이를 이용해 real 값은 0번째 imag값은 1번째에 있으므로 real, imag값을 얻을 수 있음
     */
    return std::get<1>(this->value); // real() 을 참고하여 구현할 것!
}
