...
Number *Integer::add(Number * num) {
    ...
    if (num->types() == INTEGER) {
		...
		int result = this->value + aNum->val();
		resultNum = new Integer(result);

    } else if (num->types() == FLOAT) {
		Float* aNum = dynamic_cast<Float*>(num);
		double result = this->value + aNum->val();
		resultNum = new Float(result);

    } else if (num->types() == COMPLEX) {
		Complex* aNum = dynamic_cast<Complex*>(num);
		double realSeg = this->value + aNum->val().first;
		double imagSeg = 0 + aNum->val().second;
...
Number *Integer::mul(Number * num) {
    ...
    if (num->types() == INTEGER) {
		...
		Complex* aNum = dynamic_cast<Complex*>(num);
		double realSeg = this->value * aNum->val().first;
		double imagSeg = this->value * aNum->val().second;
		resultNum = new Complex(realSeg, imagSeg);
